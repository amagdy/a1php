...
{make_link controller="group" action="search" assign="form_action"}
{form action=$form_action method="get"}

Group Name: <input type="text" name="group_search[name_like]" value="{$group_search.name_like}"/><br/>
...
<table width="100%" border="0" cellspacing="0" cellpadding="4">
	<tr class="table_title"> 
		...
		<td>
			<div align="center">
				Group Name				
				<a href="{make_link controller="group" action="search" __orderby=name __desc="no" __is_form_submitted=$__is_form_submitted array=$group_search_link}"><img src="{$HTML_ROOT}view/images/up_arrow.png" border="0"/></a>
				<a href="{make_link controller="group" action="search" __orderby=name __desc="yes" __is_form_submitted=$__is_form_submitted array=$group_search_link}"><img src="{$HTML_ROOT}view/images/down_arrow.png" border="0"/></a>
			</div>
		</td>
		<td>
			<div align="center">
				Layout				
				<a href="{make_link controller="group" action="search" __orderby=layout __desc="no" __is_form_submitted=$__is_form_submitted array=$group_search_link}"><img src="{$HTML_ROOT}view/images/up_arrow.png" border="0"/></a>
				<a href="{make_link controller="group" action="search" __orderby=layout __desc="yes" __is_form_submitted=$__is_form_submitted array=$group_search_link}"><img src="{$HTML_ROOT}view/images/down_arrow.png" border="0"/></a>
			</div>
		</td>		<td>View</td>
		<td>Edit</td>
		<td>Delete</td>
	...
	<tr bgcolor="{cycle values="#EEEEEE,#DDDDDD"}">			
		...
		<td><div align="center">{$arr_groups[groupid].layout}</div></td>		<td><div align="center"><a class="mylink" href="{make_link controller="group" action="getone" id=$arr_groups[groupid].id}">View</a></div></td>
...
<div class="div_text">Page: 
		{foreach key=k item=v from=$group.__paging_pages}
			{if $v}
			<a href="{make_link controller="group" action="search" __orderby=$__orderby __desc=$__desc __is_form_submitted=$__is_form_submitted array=$group_search_link array2=$v}" class="mylink">{$k}</a>
			{else}
